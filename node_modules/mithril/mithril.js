/* jshint -W033 */
// Search for method `m.render` which is the only one changed in this file



// This is based on the version of Mithril of Feb 18th, 2015:
// https://github.com/lhorie/mithril.js/blob/550fe9871aede73bd065da7b08f315e224aeb873/mithril.js

var Mithril = m = (function app(window, undefined) {
	...
	m.render = function(root, cell, forceRecreation) {
				// If reviver present, call it.
		// This would only happen if Mithril had been served by the server middleware
		// Expose other useful functions or variables
		// Since the reviver needs to be called only once
		// kill the reference once done.
		if (m.$reviver) {
			m.$cell = cell;
			m.$routeUnobtrusive = routeUnobtrusive;
			m.$autoRedraw = autoredraw;
			m.$reviver(cellCache, root, cell, controllers[0]);
			m.$reviver = false;
		}
		...
		//		console.log('\n\nvar cellCacheM_pre = ', JSON.stringify(cellCache[id], function (key, value) {
		//			if (key == 'nodes') return value.length + ' [' + value.map(function (node) { return node.nodeName || node.nodeValue;}) + ']';
		//			return value;
		//		}));

		cellCache[id] = build(node, null, undefined, undefined, cell, cellCache[id], false, 0, null, undefined, configs);
		//		console.log('\n\nvar cellCacheM_post = ', JSON.stringify(cellCache[id], function (key, value) {
		//			if (key == 'nodes') return value.length + ' [' + value.map(function (node) { return node.nodeName || node.nodeValue;}) + ']';
		//			return value;
		//		}));

		for (var i = 0, len = configs.length; i < len; i++) configs[i]()
		//		m.$cellCache = cellCache;
		//		m.$cell = cell;
